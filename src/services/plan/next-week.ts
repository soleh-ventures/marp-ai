...
export async function generateNextWeekPlan(input: NextWeekPlanInput): Promise<Plan> {
  ...
  const userPayload =
    ...
    `# Task\nRead this runner's RECENT performance and condition, then design ONE upcoming week (a single week in the weeks array) that is the smart next step from where they are. Return ONLY the JSON described in your instructions — no markdown, no commentary.`;
